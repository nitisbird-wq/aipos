...
- **Gotchas.** Install with `npm ci --include=optional` so the `@tailwindcss/oxide` native binding is present (CI enforces this; a plain install from an incomplete lockfile can omit it and break the build). Do not run `npm run build` while `npm run dev` is running — production build can corrupt `apps/web/.next` and make the Turbopack dev server return 500s until you stop the server, remove `.next`, and restart `npm run dev`. `npm run doctor` writes a gitignored `AIPOS_AUDIT_REPORT.md` — leave it untracked. `test:e2e` (Playwright) needs browsers installed (`npx playwright install`) and is not part of the default check set.

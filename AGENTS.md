...
- **Developer Control Center.** Prefer `npm run status` (PR merge SoT = `gh pr view --json`), `npm run services:up` / `services:down`, `npm run test:api`, `npm run verify`. See `docs/DEVELOPER_CONTROL_CENTER.md`. On Windows install `gh` yourself (`winget install --id GitHub.cli`); agents must not auto-install it.
